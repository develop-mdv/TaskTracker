...
Cron для повторяющихся задач теперь настраивается автоматически при деплое через
[`scripts/setup-cron.sh`](scripts/setup-cron.sh). GitHub Actions запускает этот
скрипт после перезапуска контейнеров.

Скрипт:
- читает `CRON_SECRET` из `.env`;
- ставит managed-блок в `crontab` без удаления чужих cron-записей;
- проверяет доступность `/api/cron/recurrence` с авторизацией;
- сразу один раз запускает генерацию ожидающих регулярных задач после деплоя.

Для ручной проверки на VPS:

```bash
cd ~/TaskTracker
./scripts/setup-cron.sh
crontab -l | sed -n '/BEGIN TaskTracker managed cron/,/END TaskTracker managed cron/p'
```

Managed-блок выглядит так:

```cron
# BEGIN TaskTracker managed cron
# Generated by scripts/setup-cron.sh. Safe to rerun on every deploy.
*/5 * * * * curl -fsS --max-time 30 -X POST 'http://localhost:3000/api/cron/recurrence' -H 'Authorization: Bearer YOUR_CRON_SECRET' >> '/path/to/TaskTracker/logs/cron/recurrence.log' 2>&1
0 3 * * 0 curl -fsS --max-time 60 -X POST 'http://localhost:3000/api/cron/cleanup' -H 'Authorization: Bearer YOUR_CRON_SECRET' >> '/path/to/TaskTracker/logs/cron/cleanup.log' 2>&1
# END TaskTracker managed cron
```

Системные cron-задачи, если они нужны, остаются отдельными ручными записями:
